 ...
 * dihitung dari titik lock (Prefs) dengan rumus jitter yang sama (clamp vektor).
...
class ChipTelemetry(
    ...
    private val sims = mutableMapOf<String, JitterSim>()
    ...
    private fun register(id: String, rootId: Int, nameId: Int, coordId: Int, extraId: Int) {
    ...
    fun onTick() {
        Targets.all.forEach { t ->
            ...
            val (oLat, oLng) = sim.advance(lock.first, lock.second)
            val lat = lock.first + oLat
            val lng = lock.second + oLng
            chip.coord.text = String.format(Locale.US, "%.6f, %.6f", lat, lng)

            val acc = (4f + sim.currentOffsetMeters(lock.first) * 1.2f).coerceIn(4f, 12f)
            val spd = sim.currentSpeed()
            val brg = sim.currentBearing()
            val alt = 20.0 + sim.currentOffsetMeters(lock.first) * 0.5
